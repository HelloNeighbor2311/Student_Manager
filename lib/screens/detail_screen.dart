...
class _DetailScreenState extends State<DetailScreen> {
  ...
  void _confirmDelete(BuildContext context) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('Bạn có chắc chắn muốn xóa sinh viên ${_currentStudent.name}?'),
        actions: [
          TextButton(
              ...
              child: Column(
                children: [
                  ...
                  Text(
                    ...
                    style: TextStyle(fontSize: 16, color: Colors.grey[600], letterSpacing: 1.1),
                  ),
                ],
              ),
            ),
  ...
  Widget _buildPersonalInfo() {
    return ListView(
      ...
      children: [
        _InfoTile(label: 'Ngày sinh', value: DateFormat('dd/MM/yyyy').format(_currentStudent.birthDate), icon: Icons.cake_outlined),
        _InfoTile(label: 'Giới tính', value: _currentStudent.gender.name == 'male' ? 'Nam' : 'Nữ', icon: Icons.person_outline),
        _InfoTile(label: 'Địa chỉ', value: _currentStudent.address, icon: Icons.location_on_outlined),
      ],
    );
  }
  ...
  Widget _buildAcademicInfo() {
    ...
    return ListView(
      padding: const EdgeInsets.all(16),
      children: [
        _InfoTile(label: 'Khoa', value: _currentStudent.department, icon: Icons.account_balance_outlined),
        _InfoTile(label: 'Ngành', value: _currentStudent.major, icon: Icons.school_outlined),
        _InfoTile(label: 'Lớp', value: _currentStudent.className, icon: Icons.class_outlined),
        _InfoTile(label: 'Khóa học', value: _currentStudent.course, icon: Icons.calendar_today_outlined),
        ...
        _InfoTile(
          ...
          trailing: Container(
            ...
            child: Text(
              _getRankText(rank),
              style: TextStyle(color: _getRankColor(rank), fontWeight: FontWeight.bold),
            ),
          ),
        ),
      ],
    ...
    return ListView(
      ...
      children: [
        _InfoTile(
          ...
          onTap: () => _launchUrl('mailto:${_currentStudent.email}'),
        ),
        _InfoTile(
          label: 'Số điện thoại', 
          value: _currentStudent.phone, 
          icon: Icons.phone_outlined,
          onTap: () => _launchUrl('tel:${_currentStudent.phone}'),
        ),
      ],
    );
  }
  ...
  Color _getRankColor(AcademicRank rank) {
    switch (rank) {
      case AcademicRank.excellent: return const Color(0xFF0A8F5A);
      case AcademicRank.good: return const Color(0xFFC99A00);
      case AcademicRank.fair: return const Color(0xFFE67E22);
      case AcademicRank.average: return const Color(0xFF7F8C8D);
    }
  }

  String _getRankText(AcademicRank rank) {
    switch (rank) {
      case AcademicRank.excellent: return 'Xuất sắc';
      case AcademicRank.good: return 'Giỏi';
      case AcademicRank.fair: return 'Khá';
      case AcademicRank.average: return 'Trung bình';
    }
  }
}
  ...
  Widget build(BuildContext context) {
    return ListTile(
      ...
      leading: Container(
        ...
        decoration: BoxDecoration(color: const Color(0xFFEDF6F9), borderRadius: BorderRadius.circular(8)),
        child: Icon(icon, color: const Color(0xFF006D77), size: 20),
      ),
      title: Text(label, style: TextStyle(fontSize: 13, color: Colors.grey[600])),
      subtitle: Text(value, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: Color(0xFF2D3748))),
